...
def load_gdp_series() -> pd.DataFrame:
    ...
    value_column = infer_value_column(df, file_name)
    if value_column is None:
        raise ValueError(f"Could not infer GDP value column for {file_name}.")
...
def check_esif_nuts2_usability() -> str:
    """Check whether the ESIF file can be mapped to NUTS2 directly.

    This file is still required for provenance checks. If no reliable NUTS2 key exists,
    the pipeline proceeds using the historic regionalized payments dataset.
    """
...
def build_outcomes(gdp: pd.DataFrame, population: pd.DataFrame) -> pd.DataFrame:
    outcomes = gdp.merge(
        population,
        on=KEY_COLUMNS,
        how="left",
    ...
    outcomes["log_gdp_pc"] = np.where(outcomes["gdp_pc"] > 0, np.log(outcomes["gdp_pc"]), np.nan)

    outcomes = outcomes.sort_values(KEY_COLUMNS).reset_index(drop=True)
    outcomes["log_gdp_pc_l1"] = outcomes.groupby("nuts2_id", sort=False)["log_gdp_pc"].shift(1)
    outcomes["gdp_pc_growth"] = 100.0 * (outcomes["log_gdp_pc"] - outcomes["log_gdp_pc_l1"])

    keep_columns = KEY_COLUMNS + ["gdp_mio_eur", "gdp_eur", "gdp_pc", "log_gdp_pc", "gdp_pc_growth"]
...
def build_eligibility_placeholder(regions: Sequence[str]) -> pd.DataFrame:
    eligibility = pd.DataFrame(
        {
            "nuts2_id": sorted(set(regions)),
            "category_2014_2020": pd.Series([pd.NA] * len(set(regions)), dtype="string"),
        }
    )
    return eligibility
...
def build_sigma_convergence(panel: pd.DataFrame) -> pd.DataFrame:
    sigma = (
        ...
        .agg(
            sigma_log_gdp=("log_gdp_pc", "std"),
            n_regions=("log_gdp_pc", "count"),
        )
        .sort_values("year")
        .reset_index(drop=True)
    )
...
def plot_treated_vs_untreated_trends(panel: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_title("Mean GDP per Capita: Treated vs Untreated Quantiles")
    ax.set_xlabel("Year")
...
def build_dataset_pipeline(write_panel_csv: bool = True) -> Dict[str, Path]:
    ...
    skeleton = build_panel_skeleton([population, gdp, treatment, controls])
    outcomes = build_outcomes(gdp=gdp, population=population)

    eligibility = build_eligibility_placeholder(skeleton["nuts2_id"].tolist())
    ...
    print(
        "[build_dataset] Built panel_master, QA tables/figures, and sigma convergence outputs."
    )
